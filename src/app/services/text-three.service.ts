...
import { TextData } from '../models/textDataTypes';

@Injectable({
  providedIn: 'root'
})
export class TextThreeService{
  ...
  public async preload(magicContainer: ElementRef<HTMLElement>): Promise<void> {
    ...
    const typo = await this.loadFont(fontUrl);
    const particle = await this.loadTexture(particleUrl);

    const environment = new Environment(typo, particle, magicContainer);
  }
...
class Environment {
  ...
  private magicContainer: HTMLElement;
  private scene: THREE.Scene;
  private camera!: THREE.PerspectiveCamera;
  private renderer!: THREE.WebGLRenderer;
  private createParticles!: CreateParticles;

  constructor(font: Font, particle: THREE.Texture, magicContainer: ElementRef<HTMLElement>) {
    this.font = font;
    this.particle = particle;
    this.magicContainer = magicContainer.nativeElement;
  ...
  private createCamera() {
    this.camera = new THREE.PerspectiveCamera(
      65,
      this.magicContainer?.clientWidth / this.magicContainer?.clientHeight || 1,
      1,
      10000
    );
  ...
  private createRenderer() {
    ...
    this.renderer.setSize(
      this.magicContainer?.clientWidth || 0,
      this.magicContainer?.clientHeight || 0
    );
...
/*
    createText() {
      ...
      // this.geometryCopy = geoParticles;
